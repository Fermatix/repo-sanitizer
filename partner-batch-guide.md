...
```bash
# Санитайзер устанавливается из GitHub, не из PyPI
...
Перед новым запуском обновите существующую копию: `git pull --ff-only origin main`,
затем `uv sync`. Проверьте `examples/rules/VERSION`: нужна версия **1.1.0 или новее**.

В этих правилах `settings.*`, `config.*`, `application.*`, `database.*` сохраняются.
Включённый `mask_config_values: true` заменяет чувствительные значения внутри
конфигов на `REDACTED` во всей истории, сохраняя имена настроек и структуру файлов.
`.env*`, файлы ключей и перечисленные в правилах локальные overlays по-прежнему
удаляются. Разрешённые примеры (`.example`, `.sample`, `.template`, `.dist`,
`.defaults`) сохраняются и очищаются. Обработка работает и при выключенных gates.

Дополнительная обработка значений поддерживает UTF-8/CP1251 и файлы до 2 MiB;
сохраняются существующие исключения для vendor/build, переводов и lockfiles.
Неподдерживаемые форматы (например, `settings.gradle`, `settings.styl`, `config.sh`,
`database.sql`) проходят только прежние детекторы; дополнительный этап отмечает
их в `skipped_path`.
В `config_values_working_tree.json` и `config_values_history.json` записываются
счётчики изменений и пропусков без исходных значений. Пропуски нужно проверить:
они не означают, что секретов в пропущенном содержимом нет.

Чтобы восстановить конфиги, удалённые старой версией, обработайте **исходные**
репозитории заново с новым файлом состояния и отдельными выходными каталогами.
Повторный запуск на старом результате удалённые файлы не восстановит; старое
batch-состояние может пропустить уже завершённые репозитории.
...
| Файл | Содержимое |
|---|---|
| `result.json` | SHA-256 бандла (и результаты гейтов, если запуск был с `--gate`) |
